...
With the advent of style points for creative usages of the dot matrix display during labs going forward, I've decided to make a very thorough guide to running arbitrary videos / animations on the drone.
...
Now that we have a folder of image files to play, we'll need to preprocess them so that they're ready for the drone. In the drone library, there *is* a [utility function that lets you pixelate images for the dot matrix](https://mituavneo.github.io/uav-neo-library/docs/utils.html#drone_utils.pixelate_image); however, uploading your raw frameset to the drone is inefficient and sometimes prohibitively so (for bad apple, the raw 6562 frames took up a combined ~500 MB).
...
### Step 3 — Play it on the drone!

Now that we have a collection of labelled, pixelated, black-and-white frames, we can finally play our animation on the drone! Remember that we can copy our frames over to the drone using
```bash
scp -r ./{folder name}/ drone@192.168.1.{team number}:~/{remote location}
```
ex.
```bash
scp -r ./processed/ drone@192.168.1.106:~/jupyter_ws/student/labs
```
