...
class CameraReal(Camera):
    ...
    __DEPTH_TOPIC = "/depth"
    __DOWNWARD_TOPIC = "/downward_camera"
    ...
    def __init__(self):
        ...
        # subscribe to the downward camera image topic, which will call
        # __downward_callback every time the downward camera publishes data
    ...
    def __downward_callback(self, data):
        try:
            np_arr = np.frombuffer(data.data, np.uint8)  # decode jpeg image type
            cv_downward_image = cv.imdecode(np_arr, cv.IMREAD_COLOR)
        except CvBridgeError as e:
            print(e)
